    ...
    <Dialog
      ...
      >
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Typography fontSize={22} fontWeight="700">{translateLang("referral.earnWithReferral")}</Typography>
          <IconButton onClick={onClose} size="small">
            <Close />
          </IconButton>
        </Stack>
        <Typography marginTop={2} fontSize={15} lineHeight="140%" color="rgba(255, 255, 255, 0.75)">
          {translateLang("referral.earnWithReferralTip")}
        </Typography>
        <Typography marginTop={4} marginBottom={2} fontSize={15} fontWeight="600">{translateLang("referral.referralURL")}</Typography>
        <Box
          display="flex"
          alignItems="center"
          marginTop={2}
        ...
        >
          ...
          >
            {translateLang("common.copy")}
          </Typography>
        </Box>
         <Typography marginTop={4} fontSize={15} fontWeight="600">{translateLang("referral.referralList")}</Typography>
         <Stack marginTop={4} direction="row">
            <Typography color={theme.palette.content.gray1} flex={1} fontSize={{xs: "14px", md: "15px"}}>{translateLang("referral.userName")}</Typography>
            <Typography color={theme.palette.content.gray1} flex={1} fontSize={{xs: "14px", md: "15px"}} textAlign="center">{translateLang("referral.earnToken")}</Typography>
            <Typography color={theme.palette.content.gray1} flex={1} fontSize={{xs: "14px", md: "15px"}} textAlign="right">{translateLang("referral.registerDate")}</Typography>
         </Stack>
         {
