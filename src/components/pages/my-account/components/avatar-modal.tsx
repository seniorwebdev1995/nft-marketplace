...
interface Props {
  ...
  open: boolean,
  onClose: () => void,
  onAvatarUpdated: (user: IUser) => void
}

export const AvatarModal = ({ imgSrc, email, nickname, open, onClose, onAvatarUpdated }: Props) => {
  ...
  const [updateAvatar] = useMutation(UPDATE_ME);
      ...
      if (file) {
        ...
        try {
          ...
          if (data && data.data?.updateMe) {
            const updatedProfile = data.data.updateMe;
